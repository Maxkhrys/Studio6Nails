/* Admin reschedule island — same slot-picker logic as the client flow,
   but redirects to /admin on success and has no 24-hour cutoff. */
  ...
  async function loadSlots() {
    ...
    try {
      ...
      if (!res.ok || slots.length === 0) {
        slotsEl!.innerHTML =
          '<p class="slots__empty">No times available that day — try another date.</p>';
        return;
      }
      slotsEl!.innerHTML = '';
      slots.forEach((iso) => {
